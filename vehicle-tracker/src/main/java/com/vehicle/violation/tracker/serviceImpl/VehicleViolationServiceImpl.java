...
import com.vehicle.violation.tracker.endpoints.VehicleViolationTrackerController;
...
public class VehicleViolationServiceImpl implements VehicleViolationService {
	...
	Logger log = org.apache.logging.log4j.LogManager.getLogger(VehicleViolationServiceImpl.class);
	...
	public List<Vehicle> getAllVehiclesOfDeviation(List<String> vehicleNumbers) {
		...
		log.info("started ::VehicleViolationServiceImpl -> getAllVehiclesOfDeviation" );
		...
		if (vehicleNumbers != null && vehicleNumbers.size() > 0) {
			...
			for (String vehicleNum : vehicleNumbers) {
				...
				log.info("fetching from DB ::VehicleViolationServiceImpl -> getAllVehiclesOfDeviation" );
		...
		log.info("Ended  :: VehicleViolationServiceImpl -> getAllVehiclesOfDeviation" );
		return vehicleList;
	}
